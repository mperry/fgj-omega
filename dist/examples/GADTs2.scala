...
class Expr[F[T1,T2]] {
  def matchWith[T1,T2](v: Visitor[T1,T2]): F[T1,T2]
}
class Visitor[T1,T2] {
  def caseIntLit(x: Int): T1
  def caseBoolLit(x: Boolean): T2
  def casePlus(x: Expr[[T1,T2]=>T1], y: Expr[[T1,T2]=>T1]): T1
  def caseCompare(x: Expr[[T1,T2]=>T1], y: Expr[[T1,T2]=>T1]): T2
  def caseIf[F[T1,T2]](x: Expr[[T1,T2]=>T2], y: Expr[F], z: Expr[F]): F[T1,T2]
}
class IntLit extends Expr[[T1,T2]=>T1] {
  val x: Int
  def matchWith[T1,T2](v: Visitor[T1,T2]): T1 =
    v.caseIntLit(x)
}
class BoolLit extends Expr[[T1,T2]=>T2] {
  val x: Boolean
  def matchWith[T1,T2](v: Visitor[T1,T2]): T2 =
    v.caseBoolLit(x)
}
...
class If[F[T1,T2]] extends Expr[[T1,T2]=>F[T1,T2]] {
...
class Eval {
  def eval[F[T1,T2]](e: Expr[F]): F[Int,Boolean] =
    e.matchWith[Int,Boolean](new EvalVisitor(this))
}
class EvalVisitor extends Visitor[Int,Boolean] {
  val evaluator: Eval
  def caseIntLit(x: Int): Int = x
  def caseBoolLit(x: Boolean): Boolean = x
...
class Print {
  def print[F[T1,T2]](e: Expr[F]): F[String,String] =
    e.matchWith[String,String](new PrintVisitor(this))
}
class PrintVisitor extends Visitor[String,String] {
  val printer: Print
  ...
  def caseCompare(x: Expr[[T1,T2]=>T1], y: Expr[[T1,T2]=>T1]): String =
    printer.print[[T1,T2]=>T1](x).append(printer.print[[T1,T2]=>T1](y))
  def caseIf[F[T1,T2]](x: Expr[[T1,T2]=>T2], y: Expr[[T1,T2]=>String] , z: Expr[F]): F[String,String] =
...
class TestGADTs {
  ...
  def test0(): Expr[[T1,T2]=>T1] =
    new Plus(new IntLit(1), new IntLit(2))

  def test00(): Int =
    (new Eval).eval[[T1,T2]=>T1](test0())
